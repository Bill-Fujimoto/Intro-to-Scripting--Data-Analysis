...
def select_columns(my_table, col_indices):
    ...
    sub_table =[]
    for row in my_table:
        sub_row = [row[idx] for idx in col_indices]
...
def sort_by_column(my_table, col_idx):
    ...
    #return not necessary since sort() is sort in place on my_table
    return my_table
...
def test_part2_code():
    ...
    # Load our file "cancer_risk_trimmed_solution.csv"
    trimmed_risk_solution = read_csv_file("cancer_risk_trimmed_solution.csv")
    
    #compare my results table with course solution
